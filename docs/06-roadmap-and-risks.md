...
| — | *(R4/R21 doc-sync note, 2026-07-23):* this row's own progress note was never updated even though R21-continued (below) already satisfies its FIRST named mitigation for the one circuit actually in production use — `/auth/session`'s `VK_HEX` is the official PSE "Semaphore V4 Ceremony 1" multi-party trusted-setup key (300-400+ contributors, public transcript, finalized 2024-09-05), not a local single-party setup. Not independently re-verified here beyond what R21-continued already documented (file integrity + structural circuit match; the full per-contribution transcript/beacon chain was not replayed). The transparent-setup (PLONK/Halo2) alternative remains untaken and is not implied to be needed now that a real ceremony key is in place — purely a doc-consistency fix, no code change. | | | | |
...
| — | *(R7 progress, 2026-07-23, "X-Wing PQ ciphersuite" pass — Mitigated, not fully Closed):* closed the "PQ ciphersuite for groups" gap the row above named, and it turned out to be a much smaller fix than the earlier framing suggested. Re-investigated the claimed upstream blocker instead of trusting the old note: `openmls_rust_crypto` 0.5.1's `kem_mode()` has a literal `unimplemented!("XWingKemDraft6 is not supported by the RustCrypto provider.")`, but `hpke-rs-rust-crypto` (already a real dependency of that very crate) fully implements the X-Wing KEM primitive — the gap was two missing match arms in the glue between two layers of the same provider stack, not a missing cryptographic primitive, and not something requiring a custom crypto provider written from scratch. Fixed by vendoring a minimally-patched `openmls_rust_crypto` (`packages/vortic-core/vendor/openmls_rust_crypto/`, MIT, see its own README.md for the exact diff) wired via `[patch.crates-io]` — ONLY `kem_mode()` (add the missing arm) and `supports()`/`supported_ciphersuites()` (advertise the suite) changed; every other function was already written generically against the resulting HPKE config and needed no changes. A SECOND real gap surfaced only by an actual test run, not by reading either crate's feature docs in advance: `hpke-rs-rust-crypto`'s X-Wing/ML-KEM support is itself gated behind its own `experimental` Cargo feature (optional `x-wing`/`ml-kem` deps) — without it, `MlsGroup::new` failed with a generic `CryptoLibraryError` with no indication why; fixed by enabling that feature in the vendored crate's own manifest. `group.rs`'s `CIPHERSUITE` now IS `MLS_256_XWING_CHACHA20POLY1305_SHA256_Ed25519` — group messages are genuinely post-quantum-resistant (hybrid ML-KEM-768+X25519), matching the 1:1 leg (`kem.rs`) this row's own text always wanted to match. **Live-verified:** all 5 of `group.rs`'s own tests (two-member add/Welcome/application round trip, distinct ciphertext per call, export/import state round trip, non-member decrypt rejection, tampered-ciphertext rejection) pass under the real X-Wing ciphersuite in `--release` (the one debug-mode "failure" is the SAME pre-existing documented `debug_assert!` quirk in openmls's own `private_message_in.rs` this module's tests already called out before this pass, not a regression); full native suite 89/89 in release; `edge-verify-only` confirmed via `cargo tree` to have zero `openmls`/`x-wing`/`ml-kem` in its dependency graph (plane separation intact); the real `wasm-pack build:client` target (what actually ships) rebuilt clean, `build:msg`/`build:issuer` (edge-verify-only) rebuilt clean and unaffected; full `packages/ohttp` live E2E suite re-run against the rebuilt wasm bundles, 7/7. **Honestly flagged, not swept under a comment:** this is now a vendored fork of a security-sensitive dependency — a real maintenance liability (frozen, won't receive upstream fixes automatically), stated prominently in three places (this note, the vendor README, `group.rs`'s own module doc) with explicit instructions to remove it the moment `openmls_rust_crypto` ships the fix itself. **Not done:** `apps/web` UI wiring for MLS groups (unchanged, still doesn't exist), Welcome delivery plumbing over `QueueDO` (this row's other still-open item, untouched this pass). | | | | |
...
| — | *(R8 progress, 2026-07-23, "seal.rs primitive" pass — Mitigated, not fully Closed):* docs/03 §6 point 1 ("sender field encrypted to recipient") was a real `todo!()` in `packages/vortic-core/src/seal.rs`, unwired into anything — found via audit, not assumed. Implemented for real: `seal()`/`unseal()` do a fresh ephemeral-X25519-DH-to-recipient (reusing `kem.rs`'s own primitives, not reimplemented) into a ChaCha20-Poly1305 envelope carrying a length-prefixed `sender_cert` alongside the plaintext, padded so the WHOLE envelope is always exactly `2^bucket` bytes — `size_bucket_for()` deliberately mirrors `bucketing.ts`'s `validateSizeBucket` convention so a future wiring pass gets byte-identical bucket numbers on both ends with no conversion layer. Deliberately decoupled from ratchet session state (own ephemeral key per call, not the ratchet's chain key) — needed anyway since receipts (point 3) must be independent sealed messages and the very first contact message has no ratchet session yet. 8 new native tests (round-trip, power-of-two envelope length across varying real content lengths — proving indistinguishability by size within a bucket, wrong-key/tampered/truncated rejection, empty-content edge case, bucket-math parity with the TS convention) — cargo 82/84 (2 pre-existing unrelated failures: an `openmls` ciphertext-decrypt panic in `group.rs` and a machine-speed-dependent PoW timing assertion in `pow.rs`, neither touched this pass). Confirmed both build profiles: `edge-verify-only` compiles clean with `seal` entirely absent (module gated in `lib.rs`, same convention as `kem`/`ratchet`/`symmetric`), `client-full` wasm32 target compiles clean with it included. **Not done, explicitly out of scope this pass:** wiring `seal_message`/`unseal_message` into any actual message-send/receive path (`useQueueTransport.ts`, `ratchet.rs`'s envelope) or into receipts specifically — this closes the crate-level primitive gap only, per this pass's own scoping choice to avoid a larger protocol-wiring change; optional cover traffic (K6) also still untouched. | | | | |
...
| — | *(R10 progress, 2026-07-23, "storage backpressure cap" pass — Mitigated, not fully Closed):* found and closed a real gap by audit — nothing anywhere capped how many undelivered messages `QueueDO` could accumulate, or how large `ConvLogDO`/`GroupDO`'s durable logs could grow; a push flood (bug or abuse of a validly-issued capability) could otherwise run any one DO toward Cloudflare's real 10 GB ceiling with no defense. Added defensive (generous, non-protocol) caps: `QueueDO` — 10,000 messages / 64 MiB per queue, checked (after evicting already-expired entries) before every `/push` insert, `429` once hit. `ConvLogDO`/`GroupDO` — 500,000 entries / 256 MiB per log (these have no TTL by design, so the cap is an honest stopgap, not archived-to-R2 yet), `507` once hit. **Real bug also found and fixed in the same file group, not hypothetical:** `GroupDO.webSocketClose` called `ws.close()` with no try/catch — the THIRD occurrence in this codebase of a bug `QueueDO`/`ConvLogDO` had each already hit and fixed once (re-closing an already-closed socket throws in workerd); `GroupDO` had never gotten the same fix. Fixed to match its siblings. **Live-verified** (temporarily lowering each cap constant for a fast, real test, then restoring — same technique this session used for the enrollment quota test): `QueueDO` count-cap (201×3 then 429) and byte-cap (201×2 then 429) both independently tripped correctly with the exact right counters in the error body; `ConvLogDO` and `GroupDO` count-caps both tripped identically (201×3 then 507); a live WS connect+close against the patched `GroupDO` produced zero new errors in the worker log (confirmed against log timestamps — the only errors present were pre-existing stale-alarm `SQLITE_BUSY` noise from local Miniflare storage, predating the test, unrelated to this fix). **Not done:** the real long-term fix this row's mitigation list names — R2 archival for long-lived `ConvLogDO`/`GroupDO` history and genuine per-conversation *elastic* sharding — this pass only adds a hard ceiling with a clear error, not a resize/overflow mechanism. | | | | |
...
| — | *(R11 progress, 2026-07-23, "enrollment ticket gate" pass — Mitigated, not fully Closed):* found and fixed a materially worse gap than the quota itself while auditing this row: `POST /token/issue` accepted ANY caller-supplied blinded message with zero proof `/oauth/callback` had ever run — an unauthenticated blind-signing oracle, not just an unenforced quota. Fixed with a single mechanism: `/oauth/callback` now checks `enroll_count >= ENROLL_QUOTA_MAX` (20, a placeholder judgment call) BEFORE its upsert, then mints a short-lived (10 min) single-use HMAC-signed ticket (`workers/enrollment/src/ticket.ts`, new `ENROLL_TICKET_SIGNING_KEY` secret, new `spent_enroll_tickets` D1 table keyed by `jti` — spend enforced via the D1 PRIMARY KEY constraint itself, not a check-then-insert, since this Worker has no per-request serialization the way a Durable Object does). `/token/issue` now requires and spends the ticket before blind-signing. `apps/web/src/pages/AuthCallback.tsx` and `packages/ohttp/src/live.e2e.test.ts`'s `getRealCapability()` updated to carry/forge it (the latter via the same "independent forger with the real local-dev signing key" technique already established for PoW stamp testing — proves the check without bypassing it). Live-verified: missing/garbage/tampered/expired ticket → 401; valid ticket → 200 then replay → 409; real RSABSSA blind→issue→finalize chain still works end-to-end; 21 successive `/oauth/callback` calls for one synthetic account (via a temporary local mock IDM, since real Xfeatures OAuth creds aren't available to tooling) capped `enroll_count` at exactly 20 and rejected the 21st with 403; full `packages/ohttp` live E2E suite 7/7 (also fixed an unrelated stale test fixture found along the way — the two full-round-trip tests hardcoded `x-size-bucket: 0`, valid only for a 1-byte ciphertext, broken by a LATER commit's real bucket-vs-length check, `6a8d959`, unrelated to this pass). **Not done:** per-epoch nullifier rate limits (this row's third named mitigation — separate from the ticket/quota mechanism built here); `ENROLL_QUOTA_MAX=20` is a judgment call, not derived from real traffic data.
...
| — | *(R12 progress, 2026-07-23, "backup key rotation" pass — Mitigated, not fully Closed):* closed the "forward-secret key rotation" gap the row above named as explicitly optional. Design constraint that shaped it: recovery must work from the phrase alone, with zero other client-side state, so the master key (`derive_backup_key_inner`) had to stay static — real rotation instead comes from an INDEXED (not chained) per-generation data key, `packages/vortic-core/src/backup.rs`'s new `gen_key[n] = HKDF(master_key, salt="vortic-backup-gen-v1", info=be32(n))`: deliberately NOT `gen_key[n+1]=H(gen_key[n])`, because a hash chain would let anyone who recovers ONE generation's key (e.g. a memory-scraping compromise without the phrase) trivially walk forward to every later one — indexed HKDF makes each generation's key independent of every other, given only the master key is secret. `n` itself is PUBLIC (the Worker tracks it in R2 custom metadata, no secrecy needed — recovery just asks the server for the current `n` and re-derives). New `export_generation_inner`/`import_generation_inner` additionally bind `(backup_id, n)` as AEAD associated data (defense-in-depth on top of the key derivation itself). Old flat `backup_export`/`backup_import` (no generation) REMOVED outright, not kept alongside — confirmed zero live callers first (`apps/web` never wired this module at all), so replacing was a clean improvement, not a breaking change, and avoided leaving two parallel encrypted-blob schemes around. `workers/messaging`'s `/backup/:backupId` PUT/GET now carry a `generation` field; the server enforces it advances by EXACTLY 1 per write via a REAL atomic compare-and-swap (R2 `onlyIf.etagMatches` against a `head()`-fetched etag), not read-then-write — closes the specific threat of a compromised/malicious host rolling the counter back and replaying a stale `(ciphertext, generation)` pair to a client that would faithfully re-derive that generation's key from nothing but the phrase. 22 native tests (17 pre-existing + 5 new: generation independence — NOT reachable from one another by hashing forward, recovery needs only the master key plus the public generation number, AAD catches a generation mismatch even holding the key constant, AAD binds `backup_id` too, wrong-length rejection) — cargo 22/22 (`backup::` module in isolation), both build profiles unaffected (`client-full`-gated, same as before). **Live-verified against real `wrangler dev`:** first write must be generation 0 (else 409); replaying generation 0 again → 409; skipping to generation 2 → 409; the correct next generation → 200 with the new blob readable back; full `packages/ohttp` live E2E suite re-run clean (7/7) as a regression check since `workers/messaging/src/index.ts` was touched. **Honestly not done:** `apps/web` UI (still entirely unwired, unchanged from before this pass); the narrow first-ever-write race (no prior R2 object/etag to CAS against, so that ONE bootstrap write uses read-then-write, not a verified R2 "must not exist" conditional — a low-value target since it's not the repeated rotation path); and, stated as plainly as the module doc itself states it, a malicious server replaying a stale `(ciphertext, generation)` pair specifically during RECOVERY (not rotation) on a device with zero local memory of the last-seen generation is undetectable by this design — that specific rollback needs client-side state the recovery contract explicitly excludes. | | | | |
