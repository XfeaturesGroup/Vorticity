...
// PATCHED (this repo's vendor fork — see ../README.md): upstream v0.5.1 has
// `HpkeKemType::XWingKemDraft6 => unimplemented!(...)` here. `hpke-rs-rust-crypto` (already a real
// dependency, unchanged from upstream) fully implements `KemAlgorithm::XWingDraft06` — confirmed by
// reading its `pq_kem.rs` before writing this patch — so this is a one-line wiring fix, not new
// cryptography.
#[inline(always)]
...
impl OpenMlsCrypto for RustCrypto {
    // PATCHED (see kem_mode's comment above): added
    // `Ciphersuite::MLS_256_XWING_CHACHA20POLY1305_SHA256_Ed25519` to both `supports()` and
    // `supported_ciphersuites()` below — upstream v0.5.1 didn't advertise it since `kem_mode()`
